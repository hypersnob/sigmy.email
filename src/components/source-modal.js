      ...
      <Modal isOpen={modal} toggle={toggle} className={className} size="lg">
        ...
        <ModalBody className="py-0">
          <FormGroup className="m-0">
            <Label for="source">
              Copy the HTML code and paste it to your signature file
            </Label>
            <Input
              type="textarea"
              className="p-2"
              name="source"
              defaultValue={getSource(signature, template)}
              rows="6"
            />
          </FormGroup>
        </ModalBody>
        <ModalFooter>
          ...
          <Button
            ...
            onClick={() => {
              copyTextToClipboard(
                getSource(signature, template, social),
                showAlert
              );
              toggle();
            }}
